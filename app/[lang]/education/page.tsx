...
export default function EducationPage() {
  ...
  const axes = [
    { title: t('acteurs.ecoles.axes.1.title'), description: t('acteurs.ecoles.axes.1.desc') },
    { title: t('acteurs.ecoles.axes.2.title'), description: t('acteurs.ecoles.axes.2.desc') },
    { title: t('acteurs.ecoles.axes.3.title'), description: t('acteurs.ecoles.axes.3.desc') },
    { title: t('acteurs.ecoles.axes.4.title'), description: t('acteurs.ecoles.axes.4.desc') },
    { title: t('acteurs.ecoles.axes.5.title'), description: t('acteurs.ecoles.axes.5.desc') },
    { title: t('acteurs.ecoles.axes.6.title'), description: t('acteurs.ecoles.axes.6.desc') },
  ]
        ...
        <div className="max-w-6xl mx-auto px-8">
          ...
          <nav className="mb-12 md:mb-16">
            <div className="lg:hidden relative flex items-center">
              ...
              >
                {navActeurs.map((item) => (
                  <LocalizedLink key={item.href} href={item.href} className={`hover:text-black transition-colors duration-200 flex-shrink-0 ${item.href === '/education' ? 'text-black font-medium' : ''}`}>
                    {item.label}
                  </LocalizedLink>
                ))}
              </div>
            ...
            <div className="hidden lg:flex gap-8 text-sm text-gray-400">
              {navActeurs.map((item) => (
                <LocalizedLink key={item.href} href={item.href} className={`hover:text-black transition-colors duration-200 ${item.href === '/education' ? 'text-black font-medium' : ''}`}>
                  {item.label}
                </LocalizedLink>
              ))}
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-start mb-24">
            <StaggerBlock>
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-black leading-tight mb-6">{t('acteurs.ecoles.title')}</h1>
              <p className="text-xl font-medium text-gray-600 mb-10">{t('acteurs.ecoles.hero')}</p>
