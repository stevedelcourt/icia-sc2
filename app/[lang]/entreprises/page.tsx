        ...
        <div className="max-w-6xl mx-4 md:mx-auto md:px-8">
          ...
          <nav className="mb-12 md:mb-16">
            <div className="lg:hidden relative flex items-center">
              ...
              >
                {navActeurs.map((item) => (
                  <LocalizedLink key={item.href} href={item.href} className={`hover:text-black transition-colors duration-200 flex-shrink-0 ${item.href === '/entreprises' ? 'text-black font-medium' : ''}`}>
                    {item.label}
                  </LocalizedLink>
                ))}
              </div>
            ...
            <div className="hidden lg:flex gap-8 text-sm text-gray-400">
              {navActeurs.map((item) => (
                <LocalizedLink key={item.href} href={item.href} className={`hover:text-black transition-colors duration-200 ${item.href === '/entreprises' ? 'text-black font-medium' : ''}`}>
                  {item.label}
                </LocalizedLink>
              ))}
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-start mb-24">
            <StaggerBlock>
              ...
              <h1 className="text-4xl md:text-5xl  font-bold text-black leading-tight mb-6">{t('acteurs.entreprises.title')}</h1>
              <p className="text-xl font-medium text-gray-600 mb-10">{t('acteurs.entreprises.hero')}</p>
              <LocalizedLink href="/contact" className="inline-flex items-center px-10 py-4 text-lg text-white bg-black hover:bg-white hover:text-black transition-colors duration-200">{t('acteurs.shared.cta')}<ArrowRight className="ml-2" /></LocalizedLink>
            </StaggerBlock>
          ...
          <StaggerBlock delay={0.2} className="pt-16">
            <h2 className="text-sm tracking-widest text-gray-400 uppercase mb-12">{t('acteurs.shared.approche_title')}</h2>
            <div className="grid md:grid-cols-2 gap-8">
              {axes.map((axe) => (
                <div key={axe.num} className="p-10 transition-colors duration-300 group">
                  ...
                  <LocalizedLink href={axe.href} className="text-base text-black hover:underline transition-colors duration-200">{t('acteurs.entreprises.en_savoir_plus')}</LocalizedLink>
                </div>
              ))}
            </div>
          </StaggerBlock>
