...
export default function EntreprisesPage() {
  ...
  const axes = [
    { num: '01', title: t('acteurs.entreprises.approche.1.title'), description: t('acteurs.entreprises.approche.1.desc'), href: '/diagnostic' },
    { num: '02', title: t('acteurs.entreprises.approche.2.title'), description: t('acteurs.entreprises.approche.2.desc'), href: '/formations' },
    { num: '03', title: t('acteurs.entreprises.approche.3.title'), description: t('acteurs.entreprises.approche.3.desc'), href: '/transformation' },
    { num: '04', title: t('acteurs.entreprises.approche.4.title'), description: t('acteurs.entreprises.approche.4.desc'), href: '/partenaire' },
  ]
        ...
        <div className="container-mentivis">
          ...
          <nav className="mb-12 md:mb-16">
            <div className="lg:hidden relative flex items-center">
              ...
              >
                {navActeurs.map((item) => (
                  <LocalizedLink key={item.href} href={item.href} className={`hover:text-black hover:underline hover:underline-offset-4 transition-colors duration-200 flex-shrink-0 ${item.href === '/entreprises' ? 'text-black font-medium' : ''}`}>
                    {item.label}
                  </LocalizedLink>
                ))}
              </div>
            ...
            <div className="hidden lg:flex gap-8 t-caption text-secondary">
              {navActeurs.map((item) => (
                <LocalizedLink key={item.href} href={item.href} className={`hover:text-black hover:underline hover:underline-offset-4 transition-colors duration-200 ${item.href === '/entreprises' ? 'text-black font-medium' : ''}`}>
                  {item.label}
                </LocalizedLink>
              ))}
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-start mb-24">
            <StaggerBlock>
              ...
              <h1 className="t-display text-primary leading-tight mb-6">{t('acteurs.entreprises.title')}</h1>
              <p className="t-lead mb-8">{t('acteurs.entreprises.hero')}</p>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="lg:pt-20 w-full lg:w-[500px] aspect-square overflow-hidden flex-shrink-0">
              <Picture src="/images/illustrations/pme-tpe.webp" alt="PME et ETI" className="w-full h-full object-cover" width={500} height={500} />
            </motion.div>
          </div>
          </section>
          ...
          <StaggerBlock delay={0.2} className="pt-16">
            <h2 className="t-caption uppercase tracking-widest mb-12">{t('acteurs.shared.approche_title')}</h2>
            <div className="grid md:grid-cols-2 gap-8">
              {axes.map((axe) => (
                <div key={axe.num} className="p-10 transition-colors duration-300 group">
                  <span className="t-caption block mb-3">{axe.num}</span>
                  <h3 className="t-heading text-primary mb-3 group-hover:text-gray-600 transition-colors duration-200">{axe.title}</h3>
                  <p className="text-secondary mb-5">{axe.description}</p>
                  <LocalizedLink href={axe.href} className="text-base text-black hover:underline hover:underline-offset-4 transition-colors duration-200">{t('acteurs.entreprises.en_savoir_plus')}</LocalizedLink>
                </div>
              ))}
            </div>
          </StaggerBlock>
