        ...
        <div className="container-mentivis">
          ...
          <nav className="mb-12 md:mb-16">
            <div className="lg:hidden relative flex items-center">
              ...
              >
                {navActeurs.map((item) => (
                  <LocalizedLink key={item.href} href={item.href} className={`hover:text-black hover:underline hover:underline-offset-4 transition-colors duration-200 flex-shrink-0 ${item.href === '/secteurs-creatifs' ? 'text-black font-medium' : ''}`}>
                    {item.label}
                  </LocalizedLink>
                ))}
              </div>
            ...
            <div className="hidden lg:flex gap-8 t-caption text-secondary">
              {navActeurs.map((item) => (
                <LocalizedLink key={item.href} href={item.href} className={`hover:text-black hover:underline hover:underline-offset-4 transition-colors duration-200 ${item.href === '/secteurs-creatifs' ? 'text-black font-medium' : ''}`}>
                  {item.label}
                </LocalizedLink>
              ))}
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-20 items-start mb-24">
            <StaggerBlock>
              ...
              <h1 className="t-display text-primary leading-tight mb-6">{t('acteurs.creatifs.title')}</h1>
              <p className="t-lead mb-8">{t('acteurs.creatifs.hero')}</p>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="lg:pt-20 w-full lg:w-[500px] aspect-square overflow-hidden flex-shrink-0">
              <Picture src="/images/illustrations/crea.webp" alt="Industries créatives" className="w-full h-full object-cover" width={500} height={500} />
            </motion.div>
          </div>
          </section>
          ...
          <StaggerBlock delay={0.2} className="pt-16 mb-12">
            <h2 className="t-caption uppercase tracking-widest mb-8">{t('acteurs.creatifs.secteurs_title')}</h2>
            <div className="flex flex-wrap gap-3">
              {sectors.map((sector) => (
                <span key={sector} className="px-5 py-3 text-base text-secondary">{sector}</span>
              ))}
            </div>
          </StaggerBlock>
